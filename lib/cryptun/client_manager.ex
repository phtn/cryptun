defmodule Cryptun.ClientManager do
  ...
  def create_tunnel(client_pid) do
    GenServer.call(__MODULE__, {:create_tunnel, client_pid})
  end
  ...
  @impl true
  def handle_call({:create_tunnel, client_pid}, _from, state) do
    tunnel_id = generate_tunnel_id()
    
    case DynamicSupervisor.start_child(
      Cryptun.TunnelSupervisor,
      {Cryptun.Tunnel, [tunnel_id: tunnel_id]}
    ) do
      {:ok, _tunnel_pid} ->
        ...
        case Registry.lookup(Cryptun.TunnelRegistry, tunnel_id) do
          [{_tunnel_pid, _}] ->
            ...
            tunnel_info = %{
              tunnel_id: tunnel_id,
              subdomain: subdomain,
              public_url: "http://#{subdomain}.localhost:4000"
            }
